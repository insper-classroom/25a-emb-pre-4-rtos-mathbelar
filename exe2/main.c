...
void btn_2_task(void *p){  
  ...
  while (true) {
    if (!gpio_get(BTN_PIN_G)) { // significa que o botão foi pressionado
      while (!gpio_get(BTN_PIN_G)) { // o código entra em um loop que aguarda até que o botão seja liberado
        vTaskDelay(pdMS_TO_TICKS(1));
      }
